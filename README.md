...
# Or run directly
python -m document_assessor.main --input input.json --output output.json
```
...
_This section summarizes the analysis and improvements made to the project during a QA session._
...
- **Solution:** The `resolution` check in `src/criteria.py` was enhanced. The new logic performs a two-stage check:
  ...
  2. If the DPI is below the threshold, it now performs a content-based analysis using OpenCV to _estimate_ the true DPI by measuring the size of detected characters.
